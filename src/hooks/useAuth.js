...
import { DEMO_ADMIN_PROFILE } from '../utils/vehicleOptions.js'
...
export function useAuth() {
    ...
    const { data: { subscription } } = supabase.auth.onAuthStateChange(
      async (event, s) => {
        ...
        if (event === 'SIGNED_IN' && s?.user) {
          setSession(s); await hydrate(s.user)
        }
        if (event === 'SIGNED_OUT' && !isDemoRef.current) {
  ...
  // ── DEMO MODE — all 8 demo vehicles, super_admin view ─
  function demo() {
